 ...
 * what this function does is, its convert string and calculate what the number is in a normal counting.
 * Also when ever you come across a non-binary, it flage error.
 ## Problems faced 
 * Edge case 
 * Wrong importation
 * Misplacement of function and characters.
 ## How to fix it
 * I worked on the problems i encountered.e.g the edge cases, the wrong imports, the misplacements.
 ## How the code runs 
 * You can run code by typing go run. the name of file you are working on.
 ## Example of the output
 * Your output will look like this when you do the go run personal. go
    fmt.Println(BinToDecimal("101 (bin)"))  = 5
	fmt.Println(BinToDecimal("hello 10 (bin) world"))  = hello 2 world
...
### you can run it by using : go run personal.go 
...
# ELIZABETH ODEH - QUOTE FUNCTION

## What the program does

This program built the go- reloaded project which hanldes a file pipeline for input, the process and output.
the packeges i used are: fmt and strings
# How to run it with examples:
1. i removed extra spaces and normalized the word, which are  strings.split and strings.TrimSpace i call them the cleaner.
my focus is on strings, loop and condition.

## CONTRIBUTION:
i figuered out the step by step function names and types to begin with and while trying to solve the problem i debugged. i also checked my group memebers codes and corrected errors where need be.
